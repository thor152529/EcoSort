...
import { Truck, Calendar, Package, Smartphone, Sofa, Tv, ChevronRight, CheckCircle2 } from "lucide-react";
import { useI18n } from "@/lib/i18n";
import { ensureAuthenticatedUser } from "@/firebase/auth";
import { createPickupRequest } from "@/firebase/firestore";
...
export function PickupScreen() {
  ...
  const handleRequest = async () => {
    try {
      ...
      await createPickupRequest({ userId: user.uid, itemType, day, time });
      setRequested(true);
      toast.success("Pickup request saved to Firebase");
    } catch (error) {
      console.error(error);
      toast.error("Could not save pickup request. Check Firebase setup.");
    }
  };
    ...
    <div className="relative h-full bg-background pb-24">
      ...
      <div className="px-5 mt-5 space-y-4">
        <div className="bg-gradient-hero rounded-2xl p-5 text-primary-foreground shadow-glow relative overflow-hidden">
          <div className="absolute -right-6 -bottom-6 opacity-20"><Truck className="h-32 w-32" /></div>
          ...
          <button
            ...
            className="mt-4 px-4 py-2 bg-accent text-accent-foreground rounded-xl text-sm font-bold inline-flex items-center gap-1"
          >
            {requested ? <><CheckCircle2 className="h-4 w-4" /> Requested</> : <>{t("book")} <ChevronRight className="h-4 w-4" /></>}
          </button>
        </div>
            ...
            {types.map((tp) => {
              ...
              return (
                <button
                  type="button"
                  key={tp.label}
                  onClick={() => { setItemType(tp.label); setRequested(false); }}
                  className={`flex flex-col items-center gap-1.5 p-3 bg-card border rounded-2xl hover:shadow-soft ${itemType === tp.label ? "border-primary ring-1 ring-primary" : "border-border"}`}
                >
        ...
        <div className="bg-card border border-border rounded-2xl p-4">
          ...
          <div className="grid grid-cols-4 gap-2">
            {["Tue", "Wed", "Thu", "Fri"].map((d, i) => (
              <button
                type="button"
                key={d}
                onClick={() => { setDay(d); setRequested(false); }}
                className={`flex flex-col items-center py-2 rounded-xl border ${day === d ? "bg-primary text-primary-foreground border-primary" : "border-border"}`}
              >
          ...
          <div className="grid grid-cols-3 gap-2 mt-3">
            {["10 AM", "1 PM", "4 PM"].map((s) => (
              <button
                type="button"
                key={s}
                onClick={() => { setTime(s); setRequested(false); }}
                className={`text-xs py-2 rounded-xl ${time === s ? "bg-secondary text-primary font-semibold" : "bg-muted text-muted-foreground"}`}
              >
                {s}
              </button>
            ))}
